stage=2
stop_stage=3
audiocategory=Breath
outerid=0
classifier=BLSTM_deltas
cd ../../
. parse_options.sh

listsdir=LISTS/$outerid
tag=$1


augpath='none'
emp=feats  
aug_for_cls=false #true
...
if [ $stage -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    python local/train_aug_V2.py \
     ...
     -m $model_config \
     -f $feats_config \
     --featsfil cambridge_data/${audiocategory}_full_flac_vad.scp \
     --trainfil cambridge_data/${audiocategory}_label_train_SR44.scp \
     --valfil cambridge_data/${audiocategory}_label_val_SR44.scp \
...
if [ $stage -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    for outerid in $(seq 0 4);do
     ...
     --relevance_type $relev_type \
        ...
        echo "scoring done for outerid $outerid and $model..."
